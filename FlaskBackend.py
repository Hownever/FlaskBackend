...
@app.route('/todo/api/v1/tasks/getAllOrUniqueTask', methods=['GET'])
# @auth.login_required
def get_tasks():
    ...
    if request.args:
        try:
            ...
            abort(400)
    return jsonify({'task': list(map(make_public_task, tasks)), 'result': True})


@app.route('/todo/api/v1/tasks/getTaskById/<int:task_id>', methods=['GET'])
# @auth.login_required
...
@app.route('/todo/api/v1/tasks/createTask/<req_type>', methods=['POST'])
# @auth.login_required
...
@app.route('/todo/api/v1/tasks/modifyTaskById/<int:task_id>/<req_type>', methods=['PUT', 'PATCH'])
# @auth.login_required
...
@app.route('/todo/api/v1/tasks/deleteTaskById/<int:task_id>', methods=['DELETE'])
# @auth.login_required
...
@app.route('/todo/api/v1/tasks/getHead/<int:task_id>', methods=['HEAD'])
# @auth.login_required
def head_task(task_id):
    return str(task_id)


@app.route('/todo/api/v1/tasks/getError/<errno>', methods=['GET'])
# @auth.login_required
def get_error(errno):
    try:
        abort(int(errno))
...
@app.route('/todo/api/v1/tasks/returnAllHead', methods=['GET', 'POST', 'HEAD', 'DELETE', 'PUT', 'PATCH'])
# @auth.login_required
def return_header():
...
@app.route('/v1/global/<status>', methods=['GET'])
# @auth.login_required
def return_global_error(status):
    ...
    with open("global_test_{}.json".format(language), "r") as apigateway_file:
...
@app.route('/v1/tasks/orderHead/<head_key>', methods=['GET'])
# @auth.login_required
...
@app.route('/v1/tasks/backendsign/<switch>', methods=['GET'])
# @auth.login_required
...
@app.route('/todo/api/v1/tasks/returntype/<type_of_return>', methods=['GET'])
# @auth.login_required
...
@app.route('/todo/api/v1/v2/poi/PoiPortraitAsy', methods=['GET'])
# @auth.login_required
def PoiPortraitAsy():
    return redirect("http://www.jd.com")


@app.route('/todo/api/v1/v2/poi/PoiPortraitSyn', methods=['GET'])
# @auth.login_required
def PoiPortraitSyn():
...
@app.route('/todo/api/v1/task/RE1/<aps>/<bpi>/<cpl>/<dpf>/<epd>/<fpb>', methods=['POST', 'GET'])
# @auth.login_required
def test_all_type(aps, bpi, cpl, dpf, epd, fpb):
    print(aps, bpi, cpl, dpf, epd, fpb)
...
@app.route('/5BC514F59E844A4DBE81C9D5878D1FA4/jingdong.hufu.jdBindQuery', methods=['POST'])
# @auth.login_required
def test_hufu():
    print(request.query_string)
    print(request.headers)
...
# @auth.login_required
def test_ppm_3(B, C):
    return "/A/{0}/{1}".format(B, C)


@app.route('/A/<B>/<C>/<D>', methods=['GET'])
# @auth.login_required
def test_ppm_4(B, C, D):
    return "/A/{0}/{1}/{2}".format(B, C, D)


@app.route('/api/compose-image', methods=['POST'])
# @auth.login_required
def test_ll():
   ...
   # print(request.stream.read())
    ...
    print(request.files)
    print("-------------")
    upload_files=request.files['images']  #获取到上传文件的最后一个文件（用于单文件上传）
    #print(upload_files)
    print("-------------")
    # import chardet
